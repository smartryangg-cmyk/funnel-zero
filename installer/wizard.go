...
func (wizard *wizardServer) installRequest(response http.ResponseWriter, request *http.Request) {
	...
	if input.ID != "" {
		...
		item.Name = name
		item.MediaEnabled = input.EnableR2
	}
...
func (wizard *wizardServer) openCloudflareRequest(response http.ResponseWriter, request *http.Request) {
	if request.Method != http.MethodPost || !wizard.authorized(request) {
		http.NotFound(response, request)
		return
	}
	if err := openURL("https://dash.cloudflare.com/"); err != nil {
		http.Error(response, "Não foi possível abrir o painel da Cloudflare.", http.StatusInternalServerError)
		return
	}
	writeWizardJSON(response, map[string]bool{"ok": true})
}
...
func (wizard *wizardServer) install(item desktopInstallation) {
	...
	message := "Worker e banco D1 estão sendo configurados."
	if item.MediaEnabled {
		message = "Worker, banco D1 e hospedagem de VSL no R2 estão sendo configurados."
	}
	wizard.update(3, "Criando sua infraestrutura", message)
...
<dialog id="onboarding-dialog"><form class="modal" method="dialog"><span class="pill"><i class="dot"></i>PRIMEIRO ACESSO</span><h2 style="margin-top:16px">Boas-vindas à KRANO</h2><p>Este app cria e gerencia sua estrutura na sua própria conta Cloudflare. Você poderá instalar, atualizar, abrir ou remover tudo por aqui.</p><div class="steps"><div class="step"><b>1</b><div><strong>Conecte sua Cloudflare</strong><p>A autorização acontece na página oficial; a KRANO não recebe sua senha.</p></div></div><div class="step"><b>2</b><div><strong>Crie uma estrutura</strong><p>O app prepara Worker + D1 e publica o painel. A hospedagem R2 é opcional.</p></div></div><div class="step"><b>3</b><div><strong>Continue pelo painel</strong><p>Depois, gerencie páginas, vídeos, tracking e integrações no navegador.</p></div></div></div><label class="check"><input id="onboarding-tutorial" type="checkbox" checked> Mostrar tutoriais e dicas para iniciantes</label><footer><button id="onboarding-start" class="primary" value="default">Começar</button></footer></form></dialog>
<script>
...
function card(i){const installed=i.status==="installed";const media=installed?(i.mediaEnabled?'<span>VSL ATIVO</span>':'<span>VSL OPCIONAL</span>'):"";return '<div class="structure"><div><h3>'+esc(i.name)+'</h3><p>'+esc(installed?i.workerUrl:"Ainda não instalada")+'</p><div class="meta"><span>'+esc(i.profile||"perfil padrão")+'</span><span>'+esc(i.version||"nova")+'</span>'+media+'<span>'+esc(i.path)+'</span></div></div><div class="row">'+(installed?'<button data-open="'+esc(i.id)+'">Abrir</button>'+(i.mediaEnabled?'':'<button class="primary" data-media="'+esc(i.id)+'">Ativar vídeos</button>')+'<button data-update="'+esc(i.id)+'">Atualizar</button><button data-recover="'+esc(i.id)+'">Recuperar</button><button class="danger" data-remove="'+esc(i.id)+'">Remover</button>':'<button class="primary" data-update="'+esc(i.id)+'">Instalar</button>')+'</div></div>'}
function bind(){document.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>api("/api/open",{id:b.dataset.open}));document.querySelectorAll("[data-media]").forEach(b=>b.onclick=()=>{if(!confirm("Ativar a hospedagem de VSL no R2 desta estrutura? A Cloudflare pode solicitar a ativação do produto."))return;api("/api/install",{id:b.dataset.media,enableR2:true}).then(poll).catch(showError)});document.querySelectorAll("[data-update]").forEach(b=>b.onclick=()=>api("/api/install",{id:b.dataset.update}).then(poll).catch(showError));document.querySelectorAll("[data-recover]").forEach(b=>b.onclick=()=>api("/api/recover",{id:b.dataset.recover}).then(poll).catch(showError));document.querySelectorAll("[data-remove]").forEach(b=>b.onclick=()=>{removeId=b.dataset.remove;const i=current.installations.find(x=>x.id===removeId);$("#confirm-label").textContent="Digite REMOVER "+i.name;$("#remove-confirm").value="";$("#remove-dialog").showModal()})}
